<form action="{{ route('karyawan.store') }}" id="formcreateKaryawan" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group mb-3 d-none" id="region-wrapper">
        <label for="kode_region" style="font-weight: 600" class="form-label">Region</label>
...
<script>
    $(function() {
        ...
        function toggleRegion() {
            let selectedText = $('#kode_jabatan option:selected').text().toLowerCase().trim();

            if (selectedText.indexOf('regional') === 0) {
                $('#region-wrapper').removeClass('d-none');
            } else {
                $('#region-wrapper').addClass('d-none');
                $('#kode_region').val(''); // reset jika bukan jabatan regional
            }
        }

        // saat jabatan berubah
        $('#kode_jabatan').on('change', function () {
            toggleRegion();
        });

        // saat halaman pertama kali load (create / edit / old input)
        toggleRegion();

        // script existing Anda
